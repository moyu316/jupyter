...
def roi_transform(feature, box, size=(32, 180)):
    ...
    affine_matrix = cv2.getAffineTransform(src_pts.astype(np.float32), dst_pts.astype(np.float32))  # 仿射变换
    ...
    grid = torch.nn.functional.affine_grid(affine_matrix.unsqueeze(0), feature.unsqueeze(0).size()) # grid:[1,640,640,2] pytorch 中的仿射变换,用于生成仿射变换下的采样网格
    ...
    # feature_rotated_crop_img = feature_rotated.squeeze().cpu().numpy().transpose(1, 2, 0).astype(np.uint8)
    # cv2.imshow('save_crop_path', feature_rotated_crop_img)
    # cv2.waitKey()
