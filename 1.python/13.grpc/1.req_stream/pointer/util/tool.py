...
def collate_fn(batch):
    ...
    images = torch.stack(images, 0)  # 把几张单独的图片拼成一个batch
    ...
    # mapping: batch中的索引， texts:刻度上的数字， bboxes：数字框的四个坐标被resize成[1,8]的维度
    mapping = np.array(mapping)
    texts = np.array(texts)
    bboxs = np.stack(bboxs, axis=0)
    bboxs = np.concatenate([bboxs, np.ones((len(bboxs), 1))], axis=1).astype(np.float32)  # 在bbox向量后添加一列1， 也就是在每个数字框的8个坐标后增加一个元素1
