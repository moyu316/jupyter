...
# 加载模型
model_yaban_path = r'F:\PythonWork\5.classification\1.resnet\weight\best\resNet50_yaban1.pth'

class_names_yaban = ['close', 'open']
num_classes = len(class_names_yaban)
net_shell = torchvision.models.resnet50(False).eval()
layerFC = net_shell.fc.in_features  # 修改最后一层的分类输出层
net_shell.fc = torch.nn.Linear(layerFC, num_classes)  # 512 --> 2
net_shell.load_state_dict(torch.load(model_yaban_path, map_location=torch.device("cpu")))
model = net_shell
...
# 加载变量并进行预测
img_folder = r"F:\PythonWork\dataset\electrical_cabinet_all\classify\yaban1\val\open"
save_img_folder = r'F:\PythonWork\dataset\electrical_cabinet_all\classify\button\button_classify_result\Visualization\resNet50_yaban1\open'
