...
class AnchorsGenerator(nn.Module):
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        ws = (w_ratios[:, None] * scales[None, :]).view(-1)   # 3种宽的缩放比例 * 5种anchor的尺寸得到15种anchor的宽的尺寸
        hs = (h_ratios[:, None] * scales[None, :]).view(-1)   # 3种高的缩放比例 * 5种anchor的尺寸得到15种anchor的高的尺寸
    ...
    def grid_anchors(self, grid_sizes, strides):
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):  # gride_sizes:[25,38] stride:32  cell_anchor:[15,4](15个生成的anchor模板)
            ...
            shift_x = shift_x.reshape(-1)    # [950] 25*38=950
            shift_y = shift_y.reshape(-1)    # [950] 25*35=950
            ...
            shifts_anchor = shifts.view(-1, 1, 4) + base_anchors.view(1, -1, 4)   # [950,1,4] + [1,15,4] = [950,15,4]
            anchors.append(shifts_anchor.reshape(-1, 4))   # anchor:[14250,4]
    ...
    def forward(self, image_list, feature_maps):
        ...
        # 根据提供的sizes和aspect_ratios生成anchors模板
        self.set_cell_anchors(dtype, device)     # size[(32, 64, 128, 256, 512)] aspect_ratios[(0.5, 1.0, 2.0)]

        # 计算/读取所有anchors的坐标信息（这里的anchors信息是映射到原图上的所有anchors信息，不是anchors模板）
        # 得到的是一个list列表，对应每张预测特征图映射回原图的anchors坐标信息
        anchors_over_all_feature_maps = self.cached_grid_anchors(grid_sizes, strides)   # [14250，4]
...
class RPNHead(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, feature in enumerate(x):
            t = F.relu(self.conv(feature))    # x:[4,1280,25,38] -> t:[4,1280,25,38]
            logits.append(self.cls_logits(t))   # [4,15,25,38]
            bbox_reg.append(self.bbox_pred(t))  # [4,60,25,38]
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        ...
        objectness = objectness[batch_idx, top_n_idx]    # [4,2000]
        levels = levels[batch_idx, top_n_idx]            # [4,2000]
        ...
        for boxes, scores, lvl, img_shape in zip(proposals, objectness_prob, levels, image_shapes):
            ...
            boxes, scores = boxes[keep], scores[keep]   # [1598,4],  [1598]
    ...
    def forward(self,
        ...
        objectness, pred_bbox_deltas = self.head(features)   # objectness:[4,15,25,38]第二个维度15代表anchor的数量, pre_bbox_deltas:[4,60,25,38]
        ...
        num_anchors_per_level_shape_tensors = [o[0].shape for o in objectness]     # [15,25,38]
        num_anchors_per_level = [s[0] * s[1] * s[2] for s in num_anchors_per_level_shape_tensors]  # [14250] =15*25*38

        # 调整内部tensor格式以及shape
        objectness, pred_bbox_deltas = concat_box_prediction_layers(objectness, pred_bbox_deltas)   # objectness[4,15,25,38]->[57000, 1]  pred_bbox_deltas:[4,60,25,38]->[57000,4]
        ...
        proposals = proposals.view(num_images, -1, 4)   # [4,14250,4]
