...
def findContours_img(original_img, opened):
    ...
    box = np.int0(cv2.boxPoints(rect))                           # box:上面获得的最小外接矩形的四个顶点
...
def Perspective_transform(box,original_img):
    ...
    orignal_W = math.ceil(np.sqrt((box[3][1] - box[2][1])**2 + (box[3][0] - box[2][0])**2))  # 两个点坐标之间的距离
    orignal_H= math.ceil(np.sqrt((box[3][1] - box[0][1])**2 + (box[3][0] - box[0][0])**2))

    # 原图中的四个顶点,与变换矩阵， pts1与pts2中的点一一对应
...
if __name__=="__main__":
    input_dir = r"F:\PythonWork\1.detection\Detect-and-read-meters-2\pointer_img\12_5.jpg"
